    ...
    $query->select('CASE 
        WHEN (SELECT GROUP_CONCAT(DISTINCT ug.title ORDER BY ug.title DESC SEPARATOR \',\') 
          FROM #__usergroups AS ug
    		  ...
    		  WHERE ugm.user_id = mgl.userid) LIKE \'%Passivmitglied deaktiviert%\' THEN \'Passivmitglied deaktiviert\' 
        WHEN (SELECT GROUP_CONCAT(DISTINCT ug.title ORDER BY ug.title DESC SEPARATOR \',\') 
          FROM #__usergroups AS ug
    		  LEFT JOIN #__user_usergroup_map AS ugm ON ug.id = ugm.group_id
    		  WHERE ugm.user_id = mgl.userid) LIKE \'%Passivmitglied%\' THEN \'Passivmitglied\' 
        WHEN (SELECT GROUP_CONCAT(DISTINCT ug.title ORDER BY ug.title DESC SEPARATOR \',\') 
          FROM #__usergroups AS ug
    		  LEFT JOIN #__user_usergroup_map AS ugm ON ug.id = ugm.group_id
